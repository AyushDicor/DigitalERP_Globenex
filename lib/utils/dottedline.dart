...
class DottedLineCorner {
  ...
  DottedLineCorner.all(double radius)
...
class DottedLine extends StatefulWidget {
  ...
  /// 虚线边框的边角。详见 [DottedLineCorner]
  ///
  /// The corners of the dotted border. See [DottedLineCorner] for details
  final DottedLineCorner? corner;

  /// 如果设置了 [child]，[DottedLine] 将会作为 [child] 的虚线边框。
  /// 此时，[width] 和 [height] 将不再有效。
  ///
  /// If [child] is set, [DottedLine] will serve as the dotted border of [child].
  /// At this time, [width] and [height] will no longer be valid.
  final Widget? child;

  /// [DottedLine] 为开发者提供了创建虚线的能力。同时支持为一个 [Widget] 创建虚线边框。支持控制虚线的粗细，间距，以及虚线边框的边角。
  ///
  /// [DottedLine] provides developers with the ability to create dashed lines. It also supports creating a dashed border for a [Widget]. Support for controlling the thickness, spacing, and corners of the dotted border.
  DottedLine({
    Key? key,
      ...
      children.add(Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.only(
            ...
            bottomLeft:
                Radius.circular(widget.corner != null ? widget.corner!.leftBottomCorner : 0.0),
            bottomRight:
                Radius.circular(widget.corner != null ? widget.corner!.rightBottomCorner : 0.0),
          ),
        ),
        key: childKey,
        child: widget.child,
      ...
      for (int i = 0; i < count.toInt(); i++) {
        canvas.drawLine(
            ...
            startOffset.translate(
                (isHorizontal ? dottedLength! : 0), (isHorizontal ? 0 : dottedLength!)),
            paint);
        startOffset = startOffset.translate((isHorizontal ? (dottedLength! + space!) : 0),
