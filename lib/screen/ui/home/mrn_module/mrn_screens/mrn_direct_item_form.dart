...
class _MrnDirectItemFormState extends State<MrnDirectItemForm> {
  ...
  double get _amount    => (_qty * _rate) - _disc;
    ...
    return Column(children: [
      ...
      MrnCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // Godown (pre-filled from source screen, but overridable)
          _SearchableField(
            label: 'Godown *',
          ...
          Row(children: [
            Expanded(
              child: _NumField(
                ...
                hint: '0',
                decimal: true,
                hasError: _gstCtrl.text.isEmpty,
                onChanged: (_) => setState(() {}),
              ),
            ),
            const SizedBox(width: 10),
          ...
          if (_qty > 0 && _rate > 0)
            _CalcPreview(
              qty: _qty, rate: _rate,
              gstPct: _gstPct, disc: _disc,
              amount: _amount, gstAmt: _gstAmt,
              totalAmt: _totalAmt,
            ),
...
class _CalcPreview extends StatelessWidget {
  final double qty, rate, gstPct, disc, amount, gstAmt, totalAmt;
  const _CalcPreview({
    required this.qty, required this.rate, required this.gstPct,
    required this.disc, required this.amount, required this.gstAmt,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(children: [
        ...
        if (disc > 0) _row('Discount', '- ₹${disc.toStringAsFixed(2)}'),
...
// ═══════════════════════════════════════════════════════════════════════════════
// Reusable searchable field (inline bottom sheet picker)
// ═══════════════════════════════════════════════════════════════════════════════
          ...
          Expanded(
            child: filtered.isEmpty
                ...
                child: Text('No results',
                    style:
                    TextStyle(color: newTextSecondary)))
                : ListView.builder(
              controller: sc,
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(
        ...
        onChanged: onChanged,
        style:
        const TextStyle(fontSize: 13, color: newTextPrimary),
        decoration: InputDecoration(
          hintText: hint,
